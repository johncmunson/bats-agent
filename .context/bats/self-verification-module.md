...
export const strategicVerifierSchema = z.object({
  verification: z
    .array(
      z.object({
        constraint: z
          .string()
          .describe('A single explicit constraint derived from the original Question.'),
...
const { output } = await generateText({
    ...
    output: Output.object({
        ...
        description: 'Structured evaluation of an agent trajectory, including verification, strategic decision, and next-step guidance.',
        schema: strategicVerifierSchema,
    }),
});
```